 ...
 * @author sahan
 */
@WebServlet(name = "CheckOutToDatabase", urlPatterns = {"/CheckOutToDatabase"})
public class CheckOutToDatabase extends HttpServlet {
            ...
            if (user != null) {
                ...
                if (verifyUser != null) {
                    if (contentId != null && !contentId.isEmpty() && Util.isInteger(contentId)) {
                        ...
                        Status status = (Status) session.get(Status.class, CheckOutToDatabase.ACTIVE_STATUS);
                        ...
                        if (checkoutContent != null && checkoutContent.getPrice() > 0 && Util.isInteger(checkoutId)) {
                            RatingType defaultRatingType = (RatingType) session.get(RatingType.class, CheckOutToDatabase.DEFAULT_RATING_TYPE_ID);
                            ...
                            if (defaultRatingType != null) {
                                ...
                                double site_price = (checkoutContent.getPrice() * CheckOutToDatabase.SITE_CONTENT_SELLING_RATE) / 100;

                                CheckoutItems checkoutItems = new CheckoutItems();
                                checkoutItems.setCheckout(checkout);
                                checkoutItems.setMainMovie(checkoutContent);
                                checkoutItems.setPrice(checkoutContent.getPrice());
                                checkoutItems.setRate(CheckOutToDatabase.SITE_CONTENT_SELLING_RATE);
                                checkoutItems.setOwner_price(checkoutContent.getPrice() - site_price);
