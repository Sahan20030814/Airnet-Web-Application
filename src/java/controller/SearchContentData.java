 ...
 * @author sahan
 */
@WebServlet(name = "SearchContentData", urlPatterns = {"/SearchContentData"})
public class SearchContentData extends HttpServlet {
            ...
            if (movieTypeId != null) {
                ...
                if (!(mainMovieList.isEmpty() && !genreSearchList.isEmpty()) || genreSearchList.isEmpty()) {
                    ...
                    if (!(mainMovieList.isEmpty() && !countrySearchList.isEmpty()) || (genreSearchList.isEmpty() && countrySearchList.isEmpty())) {
                        ...
                        Criteria c6 = session.createCriteria(MainMovie.class);
                        Status status = (Status) session.get(Status.class, SearchContentData.ACTIVE_STATUS);
                        ...
                        if (countryId != null) {
                            ...
                            if (content != null) {
                                if (content.equals("topRatingMoviesTvShows")) {
                                    ...
                                    //latest movies
                                    c6.add(Restrictions.eq("status", status));
                                    MovieType movieType = (MovieType) session.get(MovieType.class, SearchContentData.MOVIE_TYPE_ID);
                                    c6.add(Restrictions.eq("movieType", movieType));
                                    ...
                                    //latest tv shows
                                    c6.add(Restrictions.eq("status", status));
                                    MovieType tvShowType = (MovieType) session.get(MovieType.class, SearchContentData.TV_SHOW_TYPE_ID);
