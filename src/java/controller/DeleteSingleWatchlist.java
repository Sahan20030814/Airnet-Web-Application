 ...
 * @author sahan
 */
@WebServlet(name = "DeleteSingleWatchlist", urlPatterns = {"/DeleteSingleWatchlist"})
public class DeleteSingleWatchlist extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (contentId != null && isInteger(contentId)) {
            ...
            try {
                MainMovie deletingContent = (MainMovie) session.get(MainMovie.class, Integer.parseInt(contentId));
                ...
                if (deletingContent != null) {
                    if (request.getSession() != null && request.getSession().getAttribute("user") != null) {
                        ...
                        if (verifyedUser != null) {
                            ...
                            c1.add(Restrictions.eq("mainMovie", deletingContent));
                            List<Watchlist> watchlistItemList = c1.list();

                            for (Watchlist watchlist : watchlistItemList) {
                        ...
                        if (sessionWatchlists != null) {
                            for (Watchlist sessionWatchlist : sessionWatchlists) {
                                if (sessionWatchlist.getMainMovie().getId() == deletingContent.getId()) {
                                    sessionWatchlists.remove(sessionWatchlist);
                                    break;
                                }
                            }
                            request.getSession().setAttribute("sessionWatchlist", sessionWatchlists);
                        }
                    }
                }
