...
window.addEventListener("load", async function () {
    ...
    if (searchParams.has("contentId")) {
        ...
        const response = await fetch("LoadBuyingContentViewData?contentId=" + contentId);

        if (response.ok) {
            const json = await response.json();
            if (json.status) {
                document.getElementById("go-back-btn-a").href = "single_product_view.html?id=" + json.mainContent.id;
