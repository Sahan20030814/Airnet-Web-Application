async function verifyCode() {
    ...
    const response = await fetch("AdminForgotPasswordCodeVerify", {
        method: "POST",
    ...
    if (response.ok) {
        ...
        if (json.status) {
            window.location = "admin_update_password.html";
        } else {
            if (json.message === "1") {
                document.getElementById("notification").innerHTML = "Your session was expired!";
...
async function resendCode() {
    ...
    const response = await fetch("AdminResendForgotPasswordVerification");
